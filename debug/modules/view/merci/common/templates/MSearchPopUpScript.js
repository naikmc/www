Aria.tplScriptDefinition({
	$classpath: 'modules.view.merci.common.templates.MSearchPopUpScript',
	$dependencies: [
	...
	$prototype: {
		closePopUTpl: function() {
			this.__merciFunc.hideMskOverlay();
			this.__merciFunc.removeClass(document.querySelector('.booking.sear.tablet'), "show");
			this.__merciFunc.removeClass(document.querySelector('.panel.list.triplist.tablet'), "show");
			this.__merciFunc.removeClass(document.querySelector('.timetable.tablet'), "show");
		},
		...
		__createAutocompleteSource: function(listItem, locations, itemVal, webStore, appStore) {
			...
			if (this.moduleCtrl.getModuleData().booking.MSRCH_A.siteParam.allowSmartDropDown == 'FALSE') {
		...
		__createAutoCompleteInput: function() {
			...
			var globalList = this.moduleCtrl.getModuleData().booking.MSRCH_A.globalList;
			...
			if (siteParameters.airportListA == 'TRUE' && siteParameters.airportListD == 'TRUE') {

				if (siteParameters.allowSmartDropDown == 'FALSE') {
					sourceListBegin = this.__createAutocompleteSource(globalList.airportList, rqstParams.beginLocations, DepJson, storeWebSrchValue, storeAppSrchValue);
					sourceListEnd = this.__createAutocompleteSource(globalList.airportList, rqstParams.endLocations, null, false, false);
				...
				$("#B_LOCATION_1").autocomplete({
					...
					select: function(event, ui) {
						if (siteParameters.allowSmartDropDown == 'TRUE') {
						...
						if (storeAppSrchValue || storeWebSrchValue) {
							...
							if (storeAppSrchValue) {
								window.location = siteParameters.siteAppCallback + "://?flow=searchpage/fromAirport=" + code[0];
			...
			if (siteParameters.allowMCAwards == 'TRUE' && ((rqstParams.enableDirectLogin == 'YES' && rqstParams.flow == 'DEALS_AND_OFFER_FLOW') || siteParameters.allowGuestAward == 'TRUE')) {
				...
				$.getJSON(autoCompleteURL, function(data) {
					...
					var awrdBeginList = pageObj.__createAwardAutoComplete(data, siteParameters.siteRetainSearch, DepJson, storeWebSrchValue, storeAppSrchValue);
					var awrdEndList = pageObj.__createAwardAutoComplete(data, siteParameters.siteRetainSearch, null, false, false);
		...
		$displayReady: function() {
			...
			if (this.printUI == true) {
				...
				//this.moduleCtrl.setHeaderInfo(labels.tx_merci_text_booking_advs_bookflight, rqstParams.bannerHtml, siteParams.homeURL, true);
				...
				if (!this.data.isAddPaxNav) {
					...
					if (!this.__merciFunc.isEmptyObject(rqstParams.enableDirectLogin) && rqstParams.enableDirectLogin.toUpperCase() == 'YES') {
						var onOffSwitch = document.getElementById('myonoffswitch');
						if (onOffSwitch != null) {
							// reset switch
				...
				if (rqstParams.flow == 'DEALS_AND_OFFER_FLOW') {
					...
					document.searchFormPopUp.appendChild(m_Cabin);
        ...
        var strtDate = this.moduleCtrl.getModuleData().booking.MSRCH_A.requestParam.selectedOfferBean.travelStart.split(" ");
        ...
        var endDate = this.moduleCtrl.getModuleData().booking.MSRCH_A.requestParam.selectedOfferBean.travelEnd.split(" ");
			...
			$('#datePickF').datepicker({
				...
				buttonText: "",
				beforeShow: function() {
					if (siteEnableCalMonth != null && siteEnableCalMonth.toLowerCase() == 'true' && rqstParams.request.client != null && rqstParams.request.client != '') {
						var value = '';
...
else if(!(currentRightPadding<0) )
{
		...
		onSearchClick: function(ATarg, args) {
			...
			if (isSubmit) {
				...
				var request = {
					formObj: document.getElementById('searchFormPopUp'),
					method: 'POST',
					loading: true,
					expectedResponseType: 'json',
					cb: {
		...
		__validateSearchParams: function() {
			...
			if (tripTypeEL == null) {
				...
				document.searchFormPopUp.appendChild(tripTypeEL);
			} else {
				tripTypeEL.value = tripTypeValue;
				document.searchFormPopUp.appendChild(tripTypeEL);
			}
		...
		__validateDealParams: function() {
			...
			document.searchFormPopUp.appendChild(m_bLoc);
		...
		_getDateRangeDefaultValue: function() {
			...
			if (dateRangeStoredValue == null) {
				return siteParameters.dateRangeChkboxValue;
			} else {
		...
		_selectedDates: function(month, day, dayInt, monthInt) {
			...
			var showNewDtPicker = pageObj.moduleCtrl.getModuleData().booking.MSRCH_A.siteParam.showNewDatePicker;
			var retOffsetDate = pageObj.moduleCtrl.getModuleData().booking.MSRCH_A.siteParam.returnDayRange;
