Aria.tplScriptDefinition({
	$classpath: "modules.view.merci.segments.servicing.templates.services.MServicesCatalogScript",
	$dependencies: [
	...
	$statics: {
		PAGE_BOOKMARK_NAME: 'MServicesCatalogScript',
	...
	$prototype: {
		$dataReady: function() {
			...
			var model = this.moduleCtrl.getModuleData().MServicesCatalog;
			...
			if (!this.utils.isEmptyObject(model.reply.catalog)) {
				this.moduleCtrl.setCatalog(model.reply.catalog);
			}
		...
		$displayReady: function() {
			...
			setTimeout(function() {
				myScroll = new iScroll('wrapperServices', {
					snap: 'li',
					momentum: false,
					hScrollbar: false,
					vScrollbar: false,
					...
					onScrollEnd: function() {
						that.__expandElement(myScroll.currPageX);
					}
				});
			}, 10);
			this.__expandElement(0);
		},

		$viewReady: function() {
			if (document.getElementById("mobileTabs") != null) {
				document.getElementById("mobileTabs").style.display = 'block';
			}
			
			if (typeof genericScript == 'function') {
					genericScript({
						tpl:"MServicesCatalog",
						data:this.data
					});
			}
		},
		...
		/**
		 * Code that handles the CSS effects that are seen
		 * when the carrousel is moved.
		 **/
		__expandElement: function(currentIndex) {
			var elements = document.getElementById("catalogScroller").getElementsByTagName("li");
			for (var i = 0; i < elements.length; i++) {
				var elementIndex = Number(currentIndex);
				var element = elements[i];
				if (i === elementIndex) {
					element.children[0].style.display = "block";
					element.children[1].className += " expanded";
					element.children[2].style.display = "block";
				} else {
					element.children[0].style.display = "none";
					element.children[1].className = "moreServices";
		...
		__createServicesCatalog: function() {
			...
			for (var i = 0; i < paxes.length; i++) {
				...
				for (var j = 0; j < availableServiceCategories.length; j++) {
					...
					var selections = [];
					selections = this.__getPaxSelections(paxes[i].paxNumber, catCode);
					service.selections = selections;
					services.push(service);
				}
				servicesCatalog.push(services);
			}
			this.paxServicesCatalog = servicesCatalog;
		},

		__getPaxSelections: function(paxId, categoryCode) {
			var selections = [];
			for (var i = 0; i < this.selectedServices.length; i++) {
				if (categoryCode === this.selectedServices[i].category || (categoryCode === this.selectedServices[i].code)) {
					if (!this.utils.isEmptyObject(this.selectedServices) && this.selectedServices[i].passengerIds.indexOf(paxId) >= 0) {
						var selection = {};
		...
		__getRequestedServiceInfoAmt: function(paxId, categoryCode, dataMode) {
			var pieces = 0;
			var dataInfoService = "";
			var selectedCatalog = this.paxServicesCatalog[paxId];
					...
					switch (categoryCode) {
						case 'SIT':
							...
							if (this.utils.isEmptyObject(dataInfoService)) {
								var serviceName = this.getLabel(categoryCode);
		...
		getPaymentData: function(paxId, categoryCode) {
			var paymentDetails = {};
			paymentDetails.paid = null;
			paymentDetails.toPay = null;
			paymentDetails.minPrice = null;
			var paxServices = this.paxServicesCatalog[paxId] || [];
		...
		paxScrollCallBack: function(selectedPaxIndex) {
			var paxes = this.tripplan.paxInfo.travellers;
			this.$json.setValue(this.selection, 'paxId', selectedPaxIndex);
		},
